...
exports.PriceImpactCalculatorExecutor = void 0;
const logger_1 = require("../utils/logger");
const axios_1 = __importDefault(require("axios"));
class PriceImpactCalculatorExecutor {
    type = 'priceImpactCalculator';
    name = 'Price Impact Calculator';
    description = 'Calculate and analyze price impact of token swaps with risk assessment';
    ...
    async executeTemplateMode(inputs, context) {
        try {
            logger_1.logger.info(`🔧 Configuring price impact calculator for template creation`);
            const config = {
    ...
    async execute(inputs, context) {
        try {
            ...
            logger_1.logger.info(`📊 Calculating price impact for ${inputs.amount} ${inputs.from_token} → ${inputs.to_token}`);
            const config = {
                warningThreshold: 3,
            ...
            const priceImpactAnalysis = await this.calculatePriceImpact(inputs.from_token, inputs.to_token, inputs.amount, inputs.chain_id || '1', config);
            // Get liquidity depth information
            const liquidityData = await this.getLiquidityDepth(inputs.from_token, inputs.to_token, inputs.chain_id || '1');
            ...
            if (analysis.warningLevel === 'high' || analysis.warningLevel === 'critical') {
                logger_1.logger.warn(`⚠️ High price impact detected: ${analysis.priceImpact.toFixed(2)}%`);
            }
            logger_1.logger.info(`✅ Price impact analysis completed: ${analysis.priceImpact.toFixed(2)}% impact`);
            return {
                success: true,
                outputs: result,
            ...
            logger_1.logger.error('Price impact calculation failed:', error);
            return {
                success: false,
                outputs: {},
                error: error.message,
    ...
    async getMarketData(fromToken, toToken, chainId) {
        try {
            // Get current price from 1inch
            const response = await axios_1.default.get(`https://api.1inch.dev/price/v1.1/${chainId}/${fromToken}`, {
                headers: {
            ...
            logger_1.logger.warn('Failed to fetch market data, using fallback');
            return {
                currentPrice: 1,
                estimatedPrice: 1,
                priceDifference: 0,
    ...
    async calculatePriceImpact(fromToken, toToken, amount, chainId, config) {
        try {
            // Get quote from 1inch to calculate actual execution price
            const quoteResponse = await axios_1.default.get(`https://api.1inch.dev/swap/v6.0/${chainId}/quote`, {
                headers: {
                    'Authorization': `Bearer ${process.env.ONEINCH_API_KEY}`,
                    'Accept': 'application/json'
                },
            ...
            const marketResponse = await axios_1.default.get(`https://api.1inch.dev/swap/v6.0/${chainId}/quote`, {
                headers: {
                    'Authorization': `Bearer ${process.env.ONEINCH_API_KEY}`,
                    'Accept': 'application/json'
                },
            ...
            logger_1.logger.warn('Failed to get accurate price impact, using estimation');
    ...
    estimatePriceImpact(amount) {
        // Simple heuristic based on trade size
        if (amount < 1000)
            return 0.1; // Very small impact
        if (amount < 10000)
            return 0.5; // Small impact  
        if (amount < 100000)
    ...
    async getLiquidityDepth(fromToken, toToken, chainId) {
        try {
    ...
    analyzeRiskFactors(priceImpact, liquidity, config) {
        const risks = [];
    ...
    generateRecommendations(priceImpact, riskFactors, config) {
    ...
    getWarningLevel(impact, config) {
        if (impact < 1)
            return 'low';
        if (impact < config.warningThreshold)
            return 'medium';
        if (impact < config.maxImpactThreshold)
            return 'high';
        return 'critical';
    }
}
exports.PriceImpactCalculatorExecutor = PriceImpactCalculatorExecutor;
//# sourceMappingURL=price-impact-calculator-executor.js.map