...
export class ERC20TokenExecutor implements NodeExecutor {
  ...
  async execute(inputs: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult> {
    try {
      logger.info(`🪙 Creating ERC20 token: ${inputs.name} (${inputs.symbol})`);
      ...
      logger.info(`✅ ERC20 token created successfully: ${tokenData.contractAddress}`);
      ...
      logger.error(`❌ ERC20 token creation failed:`, error);
      return {
        success: false,
        outputs: {},
