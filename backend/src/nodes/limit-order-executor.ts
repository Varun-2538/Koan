...
export class LimitOrderExecutor implements NodeExecutor {
  ...
  private async executeTemplateMode(inputs: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult> {
    ...
    const mockOrderResult: LimitOrderResult = {
  ...
  private async createLimitOrder(inputs: Record<string, any>): Promise<LimitOrderResult> {
    const { 
      from_token, 
      to_token, 
      amount, 
