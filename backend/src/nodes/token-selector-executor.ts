import { NodeExecutor, ExecutionContext, NodeExecutionResult, TokenInfo } from '../types';
...
export class TokenSelectorExecutor implements NodeExecutor {
  ...
  private async executeTemplateMode(inputs: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult> {
    try {
      logger.info(`🔧 Configuring token selector for template creation`);
      ...
      const mockTokens: TokenInfo[] = config.default_tokens.map(symbol => ({
        symbol,
  ...
  async execute(inputs: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult> {
    try {
      ...
      logger.info(`🪙 Selecting tokens: ${inputs.from_token} → ${inputs.to_token}`);

      const chainId = inputs.chain_id || '1';
      const config: TokenSelectorConfig = inputs.config || {};

      let fromToken: TokenMetadata | null = null;
      let toToken: TokenMetadata | null = null;
      ...
      let availableTokens: TokenMetadata[] = [];
      if (config.enabledTokens) {
        for (const token of config.enabledTokens) {
          try {
            ...
            logger.warn(`Failed to get metadata for token ${token}:`, error);
          }
        }
      } else {
        // Use default tokens
      ...
      if (config.includeMetadata && config.priceSource) {
        await this.enrichWithPriceData([fromToken, toToken].filter(Boolean) as TokenMetadata[], config.priceSource);
      ...
      logger.info(`✅ Token selection completed: ${availableTokens.length} tokens available`);
  ...
  private async getTokenMetadata(tokenIdentifier: string, chainId: string, config: TokenSelectorConfig): Promise<TokenMetadata> {
  ...
  private async fetchTokenMetadataFromChain(address: string, chainId: string): Promise<TokenMetadata> {
    try {
      // Use 1inch token API to get metadata
      const response = await axios.get(`https://api.1inch.dev/token/v1.2/${chainId}/custom/${address}`, {
        headers: {
      ...
      logger.warn(`Failed to fetch token metadata for ${address}:`, error.message);
  ...
  private async enrichWithPriceData(tokens: TokenMetadata[], priceSource: string): Promise<void> {
  ...
  private async enrichWithCoingeckoPrices(tokens: TokenMetadata[]): Promise<void> {
    try {
      ...
      logger.warn('Failed to fetch price data from CoinGecko:', error);
    }
  }

  private async enrichWith1inchPrices(tokens: TokenMetadata[]): Promise<void> {
    // Implementation for 1inch price data
    // This would use 1inch price API when available
    logger.info('1inch price enrichment not yet implemented');
  }
