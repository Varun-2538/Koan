import { NodeExecutor, ExecutionContext, NodeExecutionResult, TransactionRequest } from '../types';
...
export class TransactionStatusExecutor implements NodeExecutor {
  ...
  async execute(inputs: Record<string, any>, context: ExecutionContext): Promise<NodeExecutionResult> {
    try {
      logger.info(`📊 Checking transaction status: ${inputs.transaction_hash}`);
      ...
      logger.info(`✅ Transaction status retrieved: ${transaction.status}`);
      return result;
  ...
  private async getTransactionDetails(txHash: string, chainId: string): Promise<any> {
    try {
  ...
  private async getTransactionReceipt(txHash: string, chainId: string): Promise<any> {
    try {
  ...
  private async getConfirmationCount(txHash: string, chainId: string): Promise<any> {
    try {
  ...
  private async analyzeGasUsage(transaction: any, chainId: string): Promise<any> {
    try {
  ...
  private async getTransactionTimeline(txHash: string, chainId: string): Promise<any[]> {
    const now = Date.now();
  ...
  private getExplorerLink(txHash: string, chainId: string): string {
  ...
  private assessTransactionRisk(transaction: any): 'low' | 'medium' | 'high' {
    // Mock risk assessment
    return 'low';
  }

  private getTransactionRecommendations(transaction: any): string[] {
  ...
  private getCurrentGasPrice(chainId: string): number {
  ...
  private getGasRecommendations(efficiency: number, couldSaveFees: boolean): string[] {
    const recommendations: string[] = [];
  ...
  private getChainFinality(chainId: string): { confirmations: number; time: string } {
    const finality: Record<string, { confirmations: number; time: string }> = {
